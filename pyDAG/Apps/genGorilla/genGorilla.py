...
def VARDELAY(input, update, tfdelay, ftdelay, icval, state):
    ...
    if INITAS:
        output = icval
        if icval:   state   = long(tf)
        else:       state   = -long(ft+1.0)
        return output, state
    if state>=0:
        if input:
            state   = long(tf)
...
class Parameter:
    ...
    def consolidate(self, newCurve, absMin, absMax):
        ...
        for i in range(newNum):
            ...
            if newPlace<0:
                raise InputError('Table %(newCurvename)s does not overlap %(name)s' %{'newCurvename': newCurve.name, 'name':  self.name})
    ...
    def partialOut(self, begin, end):
        ...
        numOut  = 0
        defOut  = '$INPUT %(name)s  %(curveType)-2i %(numPts)-4i\n' %{'name': self.name, 'curveType': self.curveType, 'numPts': numPts}
        value   = self.val[jbegin]
    ...
    def __init__(self, mtcInput):
        ...
        while i<mtcInput.numLines:
            ...
            if not ( size==REGULARSIZE+2    or \
                ...
                raise InputError('Improper format in %(pn)s at:%(line)s' %{'pn':  mtcInput.programName, 'line': mtcInput.LineS(i).str}, 1)
...
class RandomVariable():
    ...
    def __init__(self, name, min, max=100000):
        ...
        if not RandomVariable.ranSeed:      RandomVariable.ranSeed      = RandomVariable.firstRanSeed
        else:                               RandomVariable.ranSeed      += 1
    ...
    def getValueFromPast(self, past):
        ...
        if past==self.max:  return self.min
        else:               return self.max
    def __repr__(self):
        "Print the class"
        cout =  'RandomVariable %(name)-6s min = %(min)-6.4g  max = %(max)-6.4g seed = %(seed)-6i' %{'name': self.name, 'min': self.min, 'max': self.max, 'seed': self.ranSeed}
        return cout
...
class RanDeltaVar():
    ...
    def __repr__(self):
        "Print the class"
        cout =  '%(name)-12s\n  inputV= %(inputV)s\n  riseV = %(riseV)s\n  fallV = %(fallV)s\n  holdV = %(holdV)s' \
          %{'name': self.name, 'inputV': self.__inputV, 'riseV': self.__riseV, 'fallV': self.__fallV, 'holdV': self.__holdV}
        return cout
    ...
    def updateVar(self, update, time):
        ...
        if self.__type=="NORMALCONSTANT" or self.__type=="UNIFORMCONSTANT":
            ...
            if verbose > 1:
                ...
                %{'ti': time, 'in': self.__input, 'ri': self.__rise, 'ho': self.__hold, 'fa': self.__fall, 'up': update, \
                 'hi1': self.__istate1, 'hs2': self.__state2, 'li': self.__limited, 'hb3': self.__bstate3, 'hs4': self.__state4, 'hs5': self.__state5, 'ou': self.__output}
    ...
    def MONTECARLO(self, update):
        ...
        if not heldFrozen:  state4  = self.__rise
        else:               state4  = self.__state4
        self.__state4 = state4
        if not heldFrozen:  state5  = self.__fall
        else:               state5  = self.__state5
...
class SwitchVariable(RandomVariable):
    ...
    def __repr__(self):
        cout =  '%(parent)s initialValue= %(initialValue)-12.4g' %{'parent': RandomVariable.__repr__(self), 'initialValue': self.__initialValue}
        return cout
...
class OneSwitchVariable(RandomVariable):
    ...
    def __repr__(self):
        cout =  '%(parent)s offTime = %(offTime)-6.4g' %{'parent': RandomVariable.__repr__(self), 'offTime': self.__offTime}
        return cout
...
class NormalConstantVariable(RandomVariable):
    ...
    def __init__(self, name, min, max):
        "Instantiate"
        RandomVariable.__init__(self, name, min, max)
        random = math.sqrt(-2.0*math.log10(self.rand.uniform(0, 1))) * math.cos(2.0*3.1415926*self.rand.uniform(0, 1))
        self.__value = random*(self.max-self.min) + self.min
    def __repr__(self):
        cout =  '%(parent)s value = %(value)-6.4g' %{'parent': RandomVariable.__repr__(self), 'value': self.__value}
        return cout
    ...
    def genFiles(self, tod, seed):
        ...
        while finalTime<self.ftime and tend[numFiles]> 0:
            ...
            if numFiles>=MAXFILES:
                raise InputError('Too many files (%(nf)) requested, there are too many breakpoints for some reason' %{'nf':  numFiles})
        ...
        for i in range(numFiles):
            ...
            scdf        = file(scdName,     'w')
            crvf        = file(crvName,     'w')
            deff.write('# %(defName)s generated by genGorilla.py\n# from %(baseN)s and %(randN)s.\n# seed= %(seed)i\n'  %{'defName': defName, 'baseN': self.base.name, 'randN': self.rand.name, 'seed': RandomVariable.firstRanSeed})
            intf.write('# %(intName)s generated by genGorilla.py\n# from %(baseN)s and %(randN)s.\n'                    %{'intName': intName, 'baseN': self.base.name, 'randN': self.rand.name})
            scdf.write('# %(scdName)s generated by genGorilla.py\n# from %(baseN)s and %(randN)s.\n'                    %{'scdName': scdName, 'baseN': self.base.name, 'randN': self.rand.name})
            crvf.write('# %(crvName)s generated by genGorilla.py\n# from %(baseN)s and %(randN)s.\nTITLE1=\'%(baseN)s\',\nTITLE2=\'%(crvName)s\',\n' %{'crvName': crvName, 'baseN': self.base.name, 'randN': self.rand.name})
            ...
            deff.close();
            print ('MESSAGE(genGorilla.py):  %(defName)s generated from %(baseN)s and %(randN)s'    %{'defName': defName, 'baseN': self.base.name, 'randN': self.rand.name})
            intf.close();
            print ('MESSAGE(genGorilla.py):  %(intName)s generated from %(baseN)s and %(randN)s'    %{'intName': intName, 'baseN': self.base.name, 'randN': self.rand.name})
            scdf.close();
            print ('MESSAGE(genGorilla.py):  %(scdName)s generated from %(baseN)s and %(randN)s'    %{'scdName': intName, 'baseN': self.base.name, 'randN': self.rand.name})
            crvf.close();
            print ('MESSAGE(genGorilla.py):  %(crvName)s generated from %(baseN)s and %(randN)s'    %{'crvName': intName, 'baseN': self.base.name, 'randN': self.rand.name})
...
def main(argv):
    ...
    try:
        options, remainder = getopt.getopt(argv, 'c:ho:Vv:', ['calibrating=', 'help', 'output=', 'version', 'verbose=',])
    ...
    # Generate output
    tod = time.time()
    if calibrating: compProfile.genFiles(firstSeed, firstSeed)
    else:           compProfile.genFiles(tod,  firstSeed)
